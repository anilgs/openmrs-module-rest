...
public class UserConverter implements Converter {
	...
	public void marshal(Object value, HierarchicalStreamWriter writer,
		...
		writer.addAttribute("userId", nullSafeString(user.getUserId()));
		writer.addAttribute("systemId", user.getSystemId());
		writer.addAttribute("username", user.getUsername());

		Person userPerson = user.getPerson();
		if (userPerson != null) {
			if (userPerson.getBirthdate() != null)
				writer.addAttribute("birthdate", dateFormatter.format(userPerson.getBirthdate()));
			writer.addAttribute("birthdateEstimated", nullSafeString(userPerson.getBirthdateEstimated()));
			writer.addAttribute("gender", userPerson.getGender());
		...
		if (user.getRoles() != null) {
			writer.startNode("roleList");
			for (Role role: user.getRoles())
				addOptionalElement(writer, "role", role.getRole());
			writer.endNode();
		}
	}
